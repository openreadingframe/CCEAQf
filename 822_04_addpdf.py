...
# 网易云课堂
# https://study.163.com/course/courseMain.htm?courseId=1209232809&share=1&shareId=1397157280
# 文档
...
class EditorPlus(Tk):
    ...
    def _set_window_(self):
        ...
        self.protocol('WM_DELETE_WINDOW', self.exit_editor) ##退出提醒
    ...
    def _create_menu_bar_(self):
        ...
        ## 功能菜单
        ablility_menu = Menu(menu_bar, tearoff=0)
        ablility_menu.add_command(label='pdf转图片', command=lambda: popframe.pop_frame(self, "pdf转图片"))
        ablility_menu.add_command(label='下载pdf', command=lambda: popframe.pop_frame(self, "下载pdf"))

        menu_bar.add_cascade(label='功能', menu=ablility_menu)


        # 视图菜单
        about_menu = Menu(menu_bar, tearoff=0)
        about_menu.add_command(label='关于', command=lambda: self.show_messagebox('关于'))
        about_menu.add_command(label='帮助', accelerator='F1', command=lambda: self.show_messagebox('帮助'))
    ...
    def find_text(self, event=None):
        ...
        ## 窗体的位置,使之在中间
        search_toplevel.update_idletasks() ## 更新窗口大小,确保当前获得的宽度和高度是最新的
        wgt_width, wgt_height = search_toplevel.winfo_width(), search_toplevel.winfo_height() ## 获取
        ...
        search_toplevel.geometry(wm_val) ## 设定窗口大小和位置
